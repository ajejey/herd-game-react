...
export default function Lobby({ game }) {
  ...
  const copyInvite = async () => {
    ...
    // prefilled (QuickJoin handles /guesstimate/room/:code). No manual entry.
    const inviteUrl = `https://${JOIN_URL}/room/${roomCode}`;
    const text = `Join my Guesstimate game! ${inviteUrl}`;
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Guesstimate', text, url: inviteUrl });
        return;
      }
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="text-center mb-6">
        <p className="text-[#8B6347] text-base mb-1">Share this code with your friends</p>
        ...
        <button
          onClick={copyInvite}
          className="block mx-auto mt-3 bg-[#3D8B5A] hover:bg-[#2F6E45] text-white font-bold text-base px-5 py-2 rounded-full transition-colors"
        >
          {copied ? '✓ Invite copied!' : '📋 Copy invite link'}
        </button>
        <button onClick={() => setShowHelp(true)} className="mt-3 text-[#3D8B5A] hover:text-[#2F6E45] font-semibold text-base underline">
          How to play →
        </button>
      </div>
        ...
        <ul className="space-y-2">
          {players.map(p => (
            <li key={p.id} className="flex items-center justify-between">
              ...
              {isHost && p.id !== myId && (
                <button onClick={() => kickPlayer(p.id)} className="text-sm text-red-400 hover:text-red-600 font-semibold">Remove</button>
              )}
            </li>
          ))}
        </ul>
