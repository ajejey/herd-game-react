/*
  Put in Order — share text + card.
*/
import { THEME, rankFor } from './orderData';

export function buildShareText(score, isNewBest) {
  const r = rankFor(score);
  return `Put in Order — ${score} in a row\n${r.label}${isNewBest ? ' (new personal best!)' : ''}\n\nThink you can beat that? herdgamesonline.com/put-in-order`;
}

export async function buildOrderCard(score, isNewBest) {
  try {
    ...
    ctx.fillStyle = THEME.plum; ctx.fillRect(0, 0, S, 24);

    ctx.textAlign = 'center';
    ctx.fillStyle = THEME.mut;
    ctx.font = "600 40px Quicksand, sans-serif";
    ctx.fillText('Put in Order', S / 2, 150);

    ctx.fillStyle = THEME.plum;
    ...
    ctx.fillText('rounds in a row', S / 2, 530);
    ...
    ctx.fillText('herdgamesonline.com/put-in-order', S / 2, 1020);

    const blob = await new Promise((res) => canvas.toBlob(res, 'image/png'));
    if (!blob) return null;
    return new File([blob], `put-in-order-${score}.png`, { type: 'image/png' });
  } catch {
    return null;
  }
}
