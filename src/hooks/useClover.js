...
export function useClover() {
  ...
  useEffect(() => {
    ...
    socket.on('error', ({ message, code }) => {
      if (code === 'PLAYER_REMOVED') {
        clearSession(); setMyId(null); setRoomCode(null); roomCodeRef.current = null; setState(null); setKicked(true); setError(message); return;
      }
      if (code === 'ROOM_NOT_FOUND') {
        clearSession(); setMyId(null); setRoomCode(null); roomCodeRef.current = null; setState(null); setRoomNotFound(true); setErrorWithAutoClear(message); return;
      }
  ...
  const leaveGame = useCallback(() => {
    clearSession();
    setMyId(null); setRoomCode(null); roomCodeRef.current = null; setState(null); setKicked(false);
